...
export const collectionResolvers = {
  Query: {
    collections: async (_parent: any, args: QueryArgs, context: Context) => {
      ...
      try {
        // Create enhanced db with case conversion
        ...
        // All your filter conditions remain the same
    ...
    collection: async (_parent: any, { id }: { id: string }, context: Context) => {
      ...
      try {
        ...
        // Use the simpler queryFirstRow method
  ...
  Mutation: {
    createCollection: async (_parent: any, { input }: any, context: Context) => {
      ...
      try {
        ...
        // Start a transaction (using raw db to maintain transaction)
        await context.db.query('BEGIN');

        // Create the collection using the enhanced db
    ...
    `,
        ...
        // Use the first row from the camelRows property
        const collection = result.camelRows[0];

        // Initialize pulse items using the raw db for consistency
